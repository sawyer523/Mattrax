...
// binarySecurityToken is a regex used to verify a base64 encoding binary securiry token is valid
var binarySecurityToken = regexp.MustCompile(`^[a-zA-Z0-9+/]+$`)
...
func (cmd MdeGetPoliciesRequest) VerifyStructure() error {
	...
	if cmd.Header.WSSESecurity.BinarySecurityToken != "" { // Federated Authentication
		if !binarySecurityToken.MatchString(cmd.Header.WSSESecurity.BinarySecurityToken) {
			return errors.New("invalid MdeGetPoliciesRequest: invalid binary security token")
		}
		...
		return errors.New("invalid MdeGetPoliciesRequest: no supported client authentication details received")
	}
...
func (cmd MdeGetPoliciesRequest) VerifyContext(config mattrax.Config, userService types.UserService) error {
	...
	if toAddrRaw, err := url.Parse(cmd.Header.To); err != nil {
		...
		return errors.New("invalid MdeGetPoliciesRequest: this requested server ('" + strings.ToLower(toAddrRaw.Hostname()) + "') isn't this server ('" + config.WindowsDiscoveryDomain + "' or '" + config.PrimaryDomain + "')")
	}

	if cmd.Header.WSSESecurity.BinarySecurityToken != "" { // Federated Authentication
		// TODO: Verify cmd.Header.WSSESecurity.BinarySecurityToken -> All to be generated by external service
		...
		// Note: This should never happen because it should be verified in the VerifyStructure function
		return errors.New("invalid MdeGetPoliciesRequest: no supported client authentication details received")
	}
}
