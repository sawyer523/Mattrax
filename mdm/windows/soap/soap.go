...
type FaultEnvelop struct {
	...
	Reason                       FaultReasonText              `xml:"s:Body>s:Fault>s:Reason>s:Text"`
...
func (fault FaultEnvelop) Response(w http.ResponseWriter) {
	faultLogger := log.With().Str("value", fault.Value).Str("subcode", fault.Subcode).Str("reason", fault.Reason.Value).Str("", fault.Reason.Value).Str("error-type", fault.DeviceEnrollmentServiceError.Type).Str("message", fault.DeviceEnrollmentServiceError.Message).Str("traceID", fault.DeviceEnrollmentServiceError.TraceID).Logger()
...
// FaultReasonText contains the language and reason that caused the fault
